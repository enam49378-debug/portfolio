...
const PLAYLIST_RAW = [
  {
    ...
    coverSrc: 'Imagenes/Musica/La vida era mas corta.png'
  },
  {
    title:    'Solifican12',
    artist:   'Milo J',
  ...
  },
  {
    ...
    ytId:     'GCdqKFDF7ne',
    audioSrc: 'Musica/Sweater Weather.mp3',
    spotifyId: '0s5v2ItekzxC6xBTpxKVJ5',
    coverSrc: null
  }
];
...
function loadTrack(autoplay = true) {
  ...
  // Portada: usar coverSrc propio si existe, si no caer en YouTube thumbnail
  if (mpCover) {
    if (t.coverSrc) {
      mpCover.src = t.coverSrc;
    } else if (t.ytId) {
      mpCover.src = 'https://i.ytimg.com/vi/' + t.ytId + '/mqdefault.jpg';
      // Fallback si la imagen de YT falla
      mpCover.onerror = () => { mpCover.src = 'Imagenes/Musica/La vida era mas corta.png'; mpCover.onerror = null; };
    }
  }
  if (mpYtLink) {
  ...
  if (mpSpotifyLink) {
    if (t.spotifyId) {
      ...
      mpSpotifyLink.style.display = '';
    } else {
      mpSpotifyLink.style.display = 'none';
    }
  }

  renderPlaylist();

  // Avisar a script.js qué canción está sonando
  document.dispatchEvent(new CustomEvent('trackchange', { detail: { title: t.title } }));
}
